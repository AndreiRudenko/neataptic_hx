...
class Log {
    ...
    macro public static function get_level() : haxe.macro.Expr {
        return macro $v{ ${neataptic.utils.Log._level} };
    }
    macro public static function get_filter() : haxe.macro.Expr {
        return macro $v{ ${neataptic.utils.Log._filter} };
    }
    macro public static function get_exclude() : haxe.macro.Expr {
        return macro $v{ ${neataptic.utils.Log._exclude} };
    }
    ...
    macro public static function assert(expr:Expr, ?reason:ExprOf<String>) {
            ...
            return macro @:pos(Context.currentPos()) {
                if(!$expr) throw neataptic.utils.Log.DebugError.assertion( '$_str' + $reason);
            }
        #end
        return macro null;
    } //assert
    ...
    macro public static function assertnull(value:Expr, ?reason:ExprOf<String>) {
            ...
            return macro @:pos(Context.currentPos()) {
                if($value == null) throw neataptic.utils.Log.DebugError.null_assertion('$_str was null' + $reason);
            }
        #end
        return macro null;
    } //assert
